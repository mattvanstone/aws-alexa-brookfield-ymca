...
class GetFlowriderScheduleHandler(AbstractRequestHandler):
    """Handler for Skill Launch and GetNewFact Intent."""
    def can_handle(self, handler_input):
        # type: (HandlerInput) -> bool
        return (is_request_type("LaunchRequest")(handler_input) or
                is_intent_name("GetFlowriderScheduleIntent")(handler_input))

    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        logger.info("In GetFlowriderScheduleHandler")

        #random_fact = random.choice(data)
        #speech = GET_FACT_MESSAGE + random_fact
        speech = GetFlowriderSchedule()
        random_fact = speech
...
# Function to get the FlowRider schedule
def GetFlowriderSchedule():
    schedule_url = 'https://schedule.reachcm.com/IvoryRailroad/api/generalinfo.php?rinkid=52150&_=1563116882217'

    date_entry = "2019-07-27" #input('Enter a date in YYYY-MM-DD format: ')
    year, month, day = map(int, date_entry.split('-'))
    date1 = datetime.datetime(year, month, day, 0, 0, 0, 0)
    ...
    try:
        ...
        if(response.ok):
            schedule_content = response.content.decode("utf-8")
            schedule_json = json.loads(schedule_content.split("(", 1)[1].strip(")")) #json.loads(response.content)
        else:
